      ...
      <Panel>
        <div className="flex flex-col items-center px-4 py-14 text-center">
          <span className="relative mb-6 flex h-16 w-16 items-center justify-center rounded-2xl border border-white/10 bg-white/[0.03]">
            <span className="absolute inset-0 rounded-2xl bg-gradient-to-br from-sky-500/10 to-transparent" />
            <svg viewBox="0 0 24 24" className="h-7 w-7 text-sky-300" fill="none" stroke="currentColor" strokeWidth={1.6} strokeLinecap="round" strokeLinejoin="round">
              <path d={icon} />
            </svg>
          </span>
          ...
          {bullets && bullets.length > 0 ? (
            <ul className="mt-8 grid w-full max-w-md gap-2 text-left sm:grid-cols-2">
              {bullets.map((b) => (
                ...
                >
                  <svg viewBox="0 0 24 24" className="h-4 w-4 shrink-0 text-emerald-400" fill="none" stroke="currentColor" strokeWidth={2} strokeLinecap="round" strokeLinejoin="round">
